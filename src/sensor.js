...
output.gamepad = function( callback ){
    ...
    addEvent( window,'gamepadconnected', function(event){ callback(event) });
}
...
output.gyroscope = function( callback ){ 
    ...
    addEvent( window,'deviceorientation', function(event){ callback(event) } );
}

output.accelerometer = function( callback ){ 
    if( !window.DeviceMotionEvent )
        return console.error(' Accelerometer is not supported ');
    addEvent( window,'devicemotion', function(event){ callback(event) } ); 
}
